...
class PaymentService extends BaseController
{
    ...
    {
        $data = Payment::with(['branch', 'payment_type', 'sale','operator'])->paginate($request['row_count']);
    ...
    public function filterPayment($request)
    {
        ...
        if ($request['branch_id']) {
            $data = Payment::where('sale_id', $saleId)->where('payment_type_id', $paymentTypeId)->where('cash_collected_by',$operatorId)->where('branch_id', $branchId)->paginate($rowCount);
        } else {
            $data = Payment::where('sale_id', $saleId)->where('payment_type_id', $paymentTypeId)->where('cash_collected_by', $operatorId)->paginate($rowCount);
        }
